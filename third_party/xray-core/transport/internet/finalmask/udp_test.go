...
func TestUDPHeaderManagerKeepsFullReadBufferAfterShortPacket(t *testing.T) {
	cfg := &custom.UDPConfig{
		Client: []*custom.UDPItem{
			{Packet: []byte{0xAA, 0xBB}},
		},
	...
	_ = server.SetDeadline(time.Now().Add(time.Second))
	if _, err := clientRaw.WriteTo(nil, serverRaw.LocalAddr()); err != nil {
		t.Fatal(err)
	}

	payload := []byte("after-empty-datagram")
	if _, err := client.WriteTo(payload, serverRaw.LocalAddr()); err != nil {
		t.Fatal(err)
	}
